...
| Architecture specifications | Experimental contracts present; Step-1 design accepted | `Spike_A_Experimental_Snapshot.md` and `experimental-structural-input/0` remain the reviewed boundaries; `experimental-resolved-structural-model/0` received `AUDIT-STEP1-PHASE-A` `GO` and Project Owner acceptance; Canonical IR and production pipeline contracts remain undefined |
| ADRs | Incomplete | ADR0001–ADR0003 are empty; ADR0004 is Proposed; spike-only fixture-loader ADR0005 and Step-1 resolver ADR0006 are Accepted |
...
| Compiler core and canonical IR | Not implemented; experimental Step-1 design accepted | `Spike_A_Step_1_Resolution.md`, ADR0006, and `12_Structural_Step_1_Design_Review_Decision.md` define only the disposable identity/resolution boundary; Phase B remains on HOLD pending exact authorization; Canonical IR is undefined |
